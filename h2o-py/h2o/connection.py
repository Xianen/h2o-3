...
class H2OConnection(object):
  ...
  def __init__(self, ip, port, start_h2o, enable_assertions, license, max_mem_size, min_mem_size, ice_root,
               strict_version_check, proxy, nthreads, max_mem_size_GB, min_mem_size_GB):
    """
    ...
    if max_mem_size_GB is not None:
      warnings.warn("`max_mem_size_GB` is deprecated. Use `max_mem_size` instead.")
      max_mem_size = max_mem_size_GB
    if min_mem_size_GB is not None:
      warnings.warn("`min_mem_size_GB` is deprecated. Use `min_mem_size` instead.")
      min_mem_size = max_mem_size_GB
  ...
  def _start_local_h2o_jar(self, mmax, mmin, ea, license, ice, jar_path, nthreads):
    ...
    h2o_opts = ["-verbose:gc",
                ...
                + re.sub("[^A-Za-z0-9]", "_", 
                         (os.getenv("USERNAME") if sys.platform == "win32" else os.getenv("USER")) or "unknown") 
  ...
  @staticmethod
  def _tmp_file(type):
    usr = re.sub("[^A-Za-z0-9]", "_", (os.getenv("USERNAME") if sys.platform == "win32" else os.getenv("USER")) or "unknown")
    if type == "stdout":
  ...
  @staticmethod
  def proxy(): return __H2OCONN__._proxy
  ...
  def cluster_is_up(conn):
    ...
    rv = conn.current_connection()._attempt_rest(url="http://{0}:{1}/".format(conn.ip(), conn.port()), method="GET",
                                                 post_body="", file_upload_info="", proxy=conn.proxy())
  ...
  def get(url_suffix, **kwargs):
    ...
    return __H2OCONN__._do_raw_rest(url_suffix, "GET", None, proxy=__H2OCONN__._proxy, **kwargs)

  @staticmethod
  def post(url_suffix, file_upload_info=None, **kwargs):
    if __H2OCONN__ is None:
      raise ValueError("No h2o connection. Did you run `h2o.init()` ?")
    return __H2OCONN__._do_raw_rest(url_suffix, "POST", file_upload_info, proxy=__H2OCONN__._proxy, **kwargs)

  @staticmethod
  def delete(url_suffix, **kwargs):
    if __H2OCONN__ is None:
      raise ValueError("No h2o connection. Did you run `h2o.init()` ?")
    return __H2OCONN__._do_raw_rest(url_suffix, "DELETE", None, proxy=__H2OCONN__._proxy, **kwargs)
  ...
  def _rest_json(self, url_suffix, method, file_upload_info, **kwargs):
    raw_txt = self._do_raw_rest(url_suffix, method, file_upload_info, proxy=__H2OCONN__._proxy, **kwargs)
    return self._process_tables(raw_txt.json())

  # Massage arguments into place, call _attempt_rest
  def _do_raw_rest(self, url_suffix, method, file_upload_info, proxy=None, **kwargs):
    if not url_suffix:
    ...
    http_result = self._attempt_rest(url, method, post_body, file_upload_info, proxy)
  ...
  def _attempt_rest(self, url, method, post_body, file_upload_info, proxy=None):
    proxy = proxy or {}
    ...
    try:
      if method == "GET":
        ...
        return requests.post(url, files=files, headers=headers, proxies=proxy)
      elif method == "POST":
        headers["Content-Type"] = "application/x-www-form-urlencoded"
        return requests.post(url, data=post_body, headers=headers, proxies=proxy)
      elif method == "DELETE":
        return requests.delete(url, headers=headers, proxies=proxy)
      else:
