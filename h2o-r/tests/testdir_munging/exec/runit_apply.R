...
test.apply <- function() {
  hex <- h2o.importFile(h2oTest.locate("smalldata/logreg/prostate.csv"))

  h2oTest.logInfo("Perform apply on columns")
  ...
  h2oTest.logInfo("Now apply but reverse order of kalls")
  ...
  h2oTest.logInfo("Now try some misc. apply calls")
...
h2oTest.doTest("Check several cases of `apply` call ", test.apply)
